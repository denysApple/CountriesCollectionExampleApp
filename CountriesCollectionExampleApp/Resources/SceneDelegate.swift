...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, 
        ...
        let collectionVC = CountriesListViewController()

        // Embed it inside a navigation controller
        let navigationController = UINavigationController(rootViewController: collectionVC)
        
        // Create the main window and set the navigation controller as its root
        let window = UIWindow(windowScene: windowScene)
        window.backgroundColor = .white
        window.rootViewController = navigationController
        window.makeKeyAndVisible()

        // If you have a reference to the AppDelegate's window, set it here
        self.window = window
    }
